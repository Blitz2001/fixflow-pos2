...
import { Shield, Bell, Search, Command } from 'lucide-react'
...
export default async function SuperAdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[#f1f5f9] text-slate-900 font-sans">
      {/* Sidebar */}
      <SuperAdminSidebar
        ...
        <header className="h-16 border-b border-slate-200 bg-white flex items-center justify-between px-8 shrink-0 z-20 shadow-sm">
          <div className="flex items-center gap-6 flex-1">
            <div className="relative group max-w-md w-full">
              ...
              <input 
                ...
                className="w-full bg-slate-100 border-none rounded-xl pl-12 pr-4 py-2 text-sm focus:ring-2 focus:ring-brand-500 transition-all placeholder:text-slate-400"
              />
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2 px-3 py-1.5 bg-slate-100 border border-slate-200 rounded-lg">
              ...
              <span className="text-xs font-semibold text-slate-600 uppercase tracking-wide">System Admin</span>
            </div>
